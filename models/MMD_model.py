...
class Classifier_res(nn.Module):
    def __init__(self, in_dim,out_dim):
        ...
        self.fc_adapt = nn.Linear(256,256)
        self.fc1 = nn.Linear(256,128)
    ...
    def forward(self, x):
        ...
        self.feature = A_x.detach()
        return output, x
class MMD_res(BaseModel):
    def __init__(self,opt):
        super(MMD_res,self).__init__(opt)
        self.opt = opt
        self.netE = ResNet18()
        ...
        self.train_log = opt.outf + '/MMD_res_pre_train.log'
        self.model_path = opt.outf + '/MMD_res_pre_train.pth'
...
class Adapt_MMD_res(BaseModel):
    def __init__(self,opt):
        super(Adapt_MMD_res,self).__init__(opt)
        self.opt = opt
        self.source_net = opt.netS
        # self.target_net = opt.netT
        self.classifier = Classifier_res(256,10)
        self.optimizer_G = torch.optim.Adam([
            {'params': self.source_net.netE.layer2.parameters()},
            {'params': self.source_net.netE.layer3.parameters()},
            {'params': self.classifier.parameters(),'lr': opt.lr*10}], lr=opt.lr, betas=(opt.beta1, 0.999), weight_decay=opt.weight_decay)
        #self.optimizer_G = torch.optim.Adam(self.source_net.netE.parameters(),lr=opt.lr, betas=(opt.beta1, 0.999), weight_decay=opt.weight_decay)
        self.mkmmd_loss = MultipleKernelMaximumMeanDiscrepancy(
            kernels=[GaussianKernel(alpha=2 ** k) for k in range(-3, 2)],
