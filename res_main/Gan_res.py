...
#print(sys.path)
from models.GAN_ASC import Adaptation,GAN,GAN_res,Adaptation_res,Wadaptation_res
import os
import torch
...
opt.device_to_num = 1
opt.div = 10
opt.exp = 'gan20res_last_version' + opt.model
...
model_pool = {
    # 'SO': SO,
    'adapt': Adaptation,
    'Adaptation_res': Adaptation_res,
    'gan': GAN,
    'GAN_res': GAN_res,
    'Wadaptation_res': Wadaptation_res,
    # 'ADDA': ADDA,
    # 'DANN': DANN,
    # 'CUA': CUA,
}
source_model = model_pool['GAN_res'](opt)
source_model = source_model.to(opt.device)

target_model = model_pool['GAN_res'](opt)
target_model = target_model.to(opt.device)
print(source_model)
# Find total parameters and trainable parameters
total_params = sum(p.numel() for p in source_model.parameters())
print(f'{total_params:,} total parameters.')
total_trainable_params = sum(p.numel() for p in source_model.parameters() if p.requires_grad)
...
pre_train = True
if pre_train:
    source_model.load_state_dict(torch.load("/home/ydc/code2/CIDA-ASC/dump/gan20res_last_versionGAN_res/_no_adapt_gan20_res_train.pth"))
    target_model.load_state_dict(torch.load("/home/ydc/code2/CIDA-ASC/dump/gan20res_last_versionGAN_res/_no_adapt_gan20_res_train.pth"))
    opt.netS = source_model
    opt.netE = target_model
    model=model_pool['Adaptation_res'](opt)
